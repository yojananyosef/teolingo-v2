...
import { X, ChevronRight, ChevronLeft, Volume2, CheckCircle2 } from "lucide-react";
          ...
          <div className="bg-[#F7F7F7] p-6 rounded-2xl border-2 border-[#E5E5E5]">
            ...
            <div className="flex justify-around mt-4">
              ...
              <div className="text-center">
                ...
                <p className="text-xs font-bold text-[#777777] uppercase tracking-wider">Precisión</p>
              </div>
            </div>
          </div>
          <button
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <header className="flex items-center gap-4 p-4 lg:p-6 border-b-2 border-[#E5E5E5]">
        <button onClick={() => router.back()} className="text-[#AFAFAF] hover:text-[#4B4B4B] transition-colors">
          <X size={24} />
        </button>
        <div className="flex-1 h-4 bg-[#E5E5E5] rounded-full overflow-hidden">
          <div
            className="h-full bg-[#58CC02] transition-all duration-500"
            style={{ width: `${((phase - 1) * 33.33) + ((currentIdx + 1) / (phase === 1 ? vocabulary.length : sentences.length) * 33.33)}%` }}
          />
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 max-w-4xl mx-auto w-full">
        {phase === 1 && (
          <div className="w-full space-y-12 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center space-y-4">
              <h2 className="text-[#777777] font-bold text-lg uppercase tracking-widest">Fase 1: Inmersión Léxica</h2>
              <p className="text-[#4B4B4B] text-2xl font-black">Familiarízate con el sonido y la forma</p>
            </div>
            ...
            <div className="flex flex-col items-center gap-8">
              <div
                ...
                onClick={() => {
                  playHebrewText(vocabulary[currentIdx].frontContent.text);
                  setShowMeaning(true);
                }}
              >
              ...
              <p className="text-[#AFAFAF] font-bold italic">
                {showMeaning ? "Haz clic en SIGUIENTE para continuar" : "Haz clic en la tarjeta para escuchar y ver el significado"}
              </p>
            </div>
          </div>
        )}
        ...
        {phase === 2 && (
          <div className="w-full space-y-12 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center space-y-4">
              <h2 className="text-[#777777] font-bold text-lg uppercase tracking-widest">Fase 2: Mundo Cerrado</h2>
              <p className="text-[#4B4B4B] text-2xl font-black">Escucha y observa las oraciones</p>
            </div>

            <div className="flex flex-col items-center gap-8">
              <div
                className="bg-[#F7F7F7] border-4 border-[#E5E5E5] p-12 rounded-3xl w-full flex flex-col items-center gap-8"
              >
                <button
                  onClick={() => playHebrewText(sentences[currentIdx].hebrewText)}
        ...
        {phase === 3 && (
          <div className="w-full space-y-12 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center space-y-4">
              <h2 className="text-[#777777] font-bold text-lg uppercase tracking-widest">Fase 3: Traducción Guiada</h2>
              <p className="text-[#4B4B4B] text-2xl font-black">Explica el significado de la oración</p>
            </div>

            <div className="flex flex-col items-center gap-8">
              <div
                className="bg-white border-4 border-[#1CB0F6] p-12 rounded-3xl shadow-xl w-full flex flex-col items-center gap-8"
              >
                ...
                <div className="w-full space-y-4">
                  <p className="text-[#777777] font-bold text-center">¿Qué significa esta oración?</p>
                  <div className="bg-[#F7F7F7] p-6 rounded-2xl border-2 border-[#E5E5E5] text-center">
                    ...
                    <p className="text-xs text-[#AFAFAF] mt-2 font-bold uppercase tracking-wider">Pasa el cursor para ver la respuesta</p>
                  </div>
                </div>
      ...
      <footer className="p-4 lg:p-8 border-t-2 border-[#E5E5E5]">
        <div className="max-w-4xl mx-auto flex justify-between gap-4">
          ...
          >
            {phase === 3 && currentIdx === sentences.length - 1 ? "FINALIZAR" : "SIGUIENTE"} <ChevronRight size={20} />
          </button>
        </div>
      </footer>
    </div>
