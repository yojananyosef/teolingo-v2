...
export function ProfileClientContent({ user, achievements }: ProfileClientContentProps) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 lg:space-y-8 pb-24 lg:pb-12 px-4 lg:px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-6">
        <div className="bg-white p-4 lg:p-8 rounded-3xl border-2 border-[#E5E5E5] flex flex-col items-center text-center group hover:border-[#FFC800] transition-colors">
          ...
          <div className="text-[8px] lg:text-sm font-black text-[#AFAFAF] uppercase tracking-widest mt-0.5 lg:mt-2">
            {t("profile.points")}
          </div>
        </div>
      ...
      <div className="bg-white p-4 lg:p-10 rounded-3xl border-2 border-[#E5E5E5]">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-6 lg:mb-10">
          <h2 className="text-xl lg:text-3xl font-black text-[#4B4B4B] uppercase tracking-tight">
          ...
          <div className="text-[10px] lg:text-base font-black text-[#1CB0F6] bg-[#DDF4FF] px-4 lg:px-6 py-1.5 rounded-full uppercase tracking-widest">
            {achievements.filter((a) => a.isUnlocked).length} {t("profile.progressOf")} {achievements.length}
          </div>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
          {achievements.map((achievement) => {
            const nameKey = `achievements.${achievement.id}.name`;
            const descKey = `achievements.${achievement.id}.description`;
            const translatedName = t(nameKey);
            const translatedDesc = t(descKey);

            const displayName = translatedName !== nameKey ? translatedName : achievement.name;
            const displayDesc = translatedDesc !== descKey ? translatedDesc : achievement.description;
            ...
            return (
              ...
              >
                ...
                <h3
                  className={cn(
                    "font-black text-center text-[10px] lg:text-base uppercase tracking-wide leading-tight",
