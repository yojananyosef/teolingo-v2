...
import { israeliUnits, israeliVocabulary, israeliSentences, flashcards, users, userProgress, achievements, userAchievements } from "../../infrastructure/database/schema";
...
export class GetIsraeliUnitUseCase {
  async execute(unitId: string): Promise<Result<any>> {
    try {
      const [unit] = await db.select().from(israeliUnits).where(eq(israeliUnits.id, unitId)).limit(1);
...
export class CompleteIsraeliUnitUseCase {
  async execute(userId: string, _unitId: string, accuracy: number = 100): Promise<Result<any>> {
    try {
